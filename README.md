...
## Traceable offline simulation (P0)

`llmfetcher.mirofish` is an independent, offline-first simulation service for
UTF-8 TXT/Markdown seed material.  It builds a world model with stable IDs,
evidence locations, entities, relations, roles, initial events, and explicit
`input_fact` versus `model_inference` provenance.  The default provider is a
deterministic local mock: it does not require an API key and never performs a
network request.
...
`session.events` exposes the `queued → preparing → running → reporting →
completed|failed|cancelled` lifecycle.  Cancelled and failed attempts must be
explicitly reset with `service.retry(session.id)` before they can run again;
their compact prior-attempt audit data remains in `attempt_history`.  Selecting
an unregistered/real provider produces `ConfigurationError.as_dict()` before a
task is created.  Future `LLMFetcher` integration belongs behind the
`SimulationProvider` protocol; P0 deliberately registers no real provider.
